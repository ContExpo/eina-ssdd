...
const (
	...
	// Ubicar, en esta constante, nombre de fichero de vuestra clave privada
	// local emparejada con la clave pública en authorized_keys de máquinas
	// remotas
...
func TestPrimerasPruebas(t *testing.T) { // (m *testing.M) {
	// <setup code> Crear canal de resultados de ejecuciones ssh en maquinas
	// remotas
	cr := make(CanalResultados, 2000)

	// Run test sequence

	// Test1 : No debería haber ningun primario, si SV no ha recibido aún
	// latidos
	t.Run("T1:ElegirPrimerLider",
		func(t *testing.T) { cr.soloArranqueYparadaTest1(t) })

	// Test2 : No debería haber ningun primario, si SV no ha recibido aún
	// latidos
	t.Run("T1:ElegirPrimerLider",
	...
	// tear down code eliminar procesos en máquinas remotas
	cr.stop()
}
...
// start  gestor de vistas; mapa de replicas y maquinas donde ubicarlos; y lista
// clientes (host:puerto)
...
// --------------------------------------------------------------------------
// FUNCIONES DE APOYO Comprobar que hay un solo lider probar varias veces si se
// necesitan reelecciones
